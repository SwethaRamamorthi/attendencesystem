...
def get_students():
    ...
    try:
        ...
        if user_type == 'student':
            ...
            # Exclude face data field from response for performance
            students = list(students_col.find(query, {"face_data": 0}).sort('studentName', 1))
...
def get_student(student_id):
    ...
    try:
        ...
        # Get student record
        student = students_col.find_one({"_id": ObjectId(student_id)}, {"face_data": 0})
        if not student:
...
def get_all_students_admin():
    ...
    try:
        ...
        # Get all students (admin view) - exclude face data for performance
        students = list(students_col.find(query, {"face_data": 0}).sort('studentName', 1))
...
def search_students_teacher():
    ...
    try:
        ...
        students = list(students_col.find(
            query, 
            {"face_data": 0}  # Exclude face data for performance
...
def get_student_by_id_teacher(student_id_or_db_id):
    ...
    try:
        ...
        student = students_col.find_one({"studentId": student_id_or_db_id}, {"face_data": 0})
        
        if not student:
            # If not found by studentId, try by database ObjectId
            try:
                student = students_col.find_one({"_id": ObjectId(student_id_or_db_id)}, {"face_data": 0})
            except:
                pass
...
def get_student_stats():
    """Get student statistics (public access for dashboard)"""
    db = current_app.config.get("DB")
    students_col = db.students
    
    try:
        # Removed auth check - allow public access for dashboard
        # user_type = request.headers.get('X-User-Type')
        # if user_type not in ['teacher', 'admin']:
        #     return jsonify({"success": False, "error": "Unauthorized"}), 403
        ...
        face_registered = students_col.count_documents({"face_data": {"$exists": True, "$ne": None}})
