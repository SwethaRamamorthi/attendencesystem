...
# ----------------- Extract Face Data -----------------
def extract_face_data(face_img):
    try:
        face_data = DeepFace.represent(face_img, model_name='Facenet512', detector_backend='skip')
        return face_data[0]['embedding']
    except Exception as e:
        print("Error extracting face data:", e)
        return None
            ...
            if time.time() - start_time > wait_time:
                ...
                if face_data is not None:
                    user_data = {
                        'user_id': user_id,
                        'name': name,
                        'face_data': face_data.tolist() if isinstance(face_data, np.ndarray) else face_data
                    }
...
def live_recognition():
    ...
    while True:
        ...
        for face_data_item in faces:
            x, y, w, h = face_data_item['box']
            face_img = face_data_item['face']
            current_face_data = extract_face_data(face_img)
            ...
            for user in users:
                ...
                distance = cosine(current_face_data, stored_face_data)
