...
import { Camera, ArrowLeft, Play, Square, User, Calendar, BookOpen, GraduationCap, Users, CheckCircle2, Clock } from "lucide-react";
...
export default function DemoSessionPage() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    // Auto-populate subject when period is selected
    if (name === "period" && periods.length > 0) {
      const selectedPeriod = periods.find(p => p.period_number === parseInt(value));
      if (selectedPeriod) {
        setForm({ ...form, period: value, subject: selectedPeriod.subject });
      }
    }
  };

  // Fetch timetable when class details change
  useEffect(() => {
    if (form.department && form.year && form.division) {
      fetchTimetable();
    }
  }, [form.department, form.year, form.division]);
  ...
  return (
    <div className="min-h-screen bg-gradient-blue">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-4 -right-4 w-72 h-72 bg-accent-blue-100 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-float"></div>
        <div className="absolute -bottom-8 -left-4 w-72 h-72 bg-accent-blue-200 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-float" style={{ animationDelay: "2s" }}></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-72 h-72 bg-accent-blue-50 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-float" style={{ animationDelay: "4s" }}></div>
      </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                <div className="p-2 bg-gradient-to-r from-primary-blue-light to-primary-blue rounded-lg">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white/70 backdrop-blur-lg rounded-xl border-2 border-blue-200 p-6 shadow-lg">
                ...
                <div className="space-y-4">
                  ...
                  {periods.length > 0 && (
                    <div>
                      <label className="text-gray-700 text-sm mb-2 block font-medium">Period</label>
                      <select
                        name="period"
                        value={form.period}
                        onChange={handleChange}
                        className="glass-input w-full p-3 text-gray-800"
                      >
                        <option value="">Select Period</option>
                        {periods.map(p => (
                          <option key={p.period_number} value={p.period_number}>
                            Period {p.period_number} - {p.subject} ({p.start_time} - {p.end_time})
                          </option>
                        ))}
                      </select>
                    </div>
                  ...
                  <button
                    ...
                    className="glossy-button w-full py-3 px-4 flex items-center justify-center gap-3 mt-6"
                  >
